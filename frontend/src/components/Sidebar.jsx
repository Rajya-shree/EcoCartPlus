...
const Sidebar = () => {
  ...
  // 🟢 The ONLY count state we need now
  ...
  useEffect(() => {
    // 1. Fetch when the sidebar loads
    fetchNotificationCount();

    // 2. Check the database every 60 seconds
    const interval = setInterval(fetchNotificationCount, 60000);

    // 3. Listen for instant updates (like when Purge Feed or Admin Update is clicked)
    window.addEventListener("forceNotificationRefresh", fetchNotificationCount);
                  ...
                  <div className="feed-trigger-wrapper" ref={notifyRef}>
                    ...
                    >
                      ...
                      <span className="feed-badge">
                        {/* 🟢 Now properly mapped to the real-time notificationCount */}
...
//   //NEW
//   const [notificationCount, setNotificationCount] = useState(0);
//   // 🟢 NEW: Fetch the ACTUAL count from the database
//   const fetchNotificationCount = async () => {
//     if (!userInfo?.token) return;
//     try {
//       const config = { headers: { Authorization: `Bearer ${userInfo.token}` } };
...
//       // Update the badge with the exact number of unread alerts
//       setNotificationCount(data.length);
//     } catch (err) {
//       console.error("Sidebar sync failed", err);
//     }
